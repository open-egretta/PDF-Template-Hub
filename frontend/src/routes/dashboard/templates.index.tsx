...
function RouteComponent() {
  ...
  >({
    ...
    queryFn: async () => {
      return apiService.get("/templates/admin/all").then((res) => {
        const { data } = res;
        console.log(data);
        return data;
      });
  ...
  const deleteTemplate = (id: string) => {
    apiService
      ...
      .then((res) => {
        ...
        toaster.create({
          title: "刪除成功",
          description: `已刪除模板: ${res.data.name}`,
          type: "success",
        });
        query.refetch();
      })
  ...
  const toggleActive = (id: string, is_active: boolean) => {
    apiService
      .patch(`/templates/${id}/active`, { is_active })
      .then((res) => {
        toaster.create({
          title: is_active ? "已啟用" : "已停用",
          description: `模板: ${res.data.name}`,
          type: "success",
        });
        query.refetch();
      })
    ...
    <Container>
      ...
      <SimpleGrid columns={{ sm: 1, md: 3 }} gap="6">
        {query.data?.map((template) => (
          <Card.Root key={template.id} marginBottom="4">
            <Box
              cursor={"pointer"}
              onClick={() =>
                navigate({
